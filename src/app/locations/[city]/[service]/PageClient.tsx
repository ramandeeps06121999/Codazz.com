...
export default function CityServicePageClient(props: CityServicePageProps) {
  ...
  const PORTFOLIO_POOL: Record<string, { title: string; desc: string; metrics: string; gradient: string }> = {
    'FinTech': { title: 'Real-Time Payment Processing Engine for Series B Startup', desc: 'Built a PCI-DSS Level 1 compliant payment gateway handling ACH, wire, and card-present transactions with sub-200ms latency. Integrated KYC/AML screening via Plaid and Sardine APIs with automated SAR filing.', metrics: '$840M processed in first year', gradient: 'linear-gradient(135deg, rgba(34,197,94,0.15), rgba(34,197,94,0.03))' },
    ...
    'Robotics': { title: 'Warehouse Pick-and-Pack Orchestration for AMR Fleet', desc: 'Built the fleet-management layer coordinating 60 autonomous mobile robots with WMS integration, dynamic path planning, charging scheduling, and human-safety interlock protocols.', metrics: '2.8x picks per hour vs. manual process', gradient: 'linear-gradient(135deg, rgba(34,197,94,0.15), rgba(34,197,94,0.03))' },
    ...
    'FinTech & M-Pesa': { title: 'Mobile Money Agent Network Management Platform', desc: 'Built a float-management and reconciliation system for 40,000+ M-Pesa agents with real-time liquidity alerts, automated commission calculations, and fraud-detection rules on transaction velocity.', metrics: '99.4% transaction reconciliation rate', gradient: 'linear-gradient(135deg, rgba(34,197,94,0.15), rgba(34,197,94,0.03))' },
    ...
    'Banking & Finance': { title: 'Core Banking Middleware Migration for Regional Credit Union', desc: 'Replaced a COBOL-based core with an API-driven middleware layer enabling mobile banking, Zelle P2P integration, and real-time fraud scoring while maintaining backward compatibility with legacy GL systems.', metrics: 'Mobile banking adoption jumped from 22% to 68%', gradient: 'linear-gradient(135deg, rgba(34,197,94,0.15), rgba(34,197,94,0.03))' },
  ...
  const DEFAULT_PORTFOLIO = [
    { title: 'Multi-Tenant RBAC & Audit System for Compliance-Heavy SaaS', industry: 'Technology', desc: 'Architected a hierarchical permission model supporting 200+ granular scopes with SOC 2 Type II audit trails and SCIM provisioning across 3,400 enterprise accounts.', metrics: '99.97% uptime over 18 months', gradient: 'linear-gradient(135deg, rgba(34,197,94,0.15), rgba(34,197,94,0.03))' },
  ...
  const portfolioItems = localIndustries.length > 0
    ? localIndustries.slice(0, 3).map(ind => {
        ...
        return { title: `${ind} Platform`, industry: ind, desc: `Custom-built solution for ${cityName}'s ${ind.toLowerCase()} sector with industry-specific compliance and integrations.`, metrics: 'Custom scope', gradient: 'linear-gradient(135deg, rgba(34,197,94,0.15), rgba(34,197,94,0.03))' };
      })
    : DEFAULT_PORTFOLIO;
        ...
        <section ref={portfolioRef} style={{ ...sectionPad, ...sectionBorder }}>
          <div className="cb-container">
            <div className="reveal" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 64, gap: 40, flexWrap: 'wrap' }}>
              ...
              <Link href="/case-studies" style={{ fontSize: 14, fontWeight: 600, color: '#ffffff', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 8, transition: '0.2s' }}
              ...
              {portfolioItems.map((p, i) => (
                <div key={p.title} className={`reveal reveal-d${i + 1}`} style={{ ...cardStyle, padding: 0, overflow: 'hidden', borderRadius: 28 }}
                  ...
                  <div style={{ height: 200, background: p.gradient, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ width: 60, height: 60, borderRadius: 16, background: 'rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  <div className="loc-portfolio-inner" style={{ padding: '28px 32px' }}>
                    <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#ffffff', marginBottom: 10, display: 'block' }}>{p.industry}</span>
                    <h3 style={{ fontSize: 18, fontWeight: 600, color: '#ffffff', letterSpacing: '-0.02em', marginBottom: 10 }}>{p.title}</h3>
                    <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.5)', lineHeight: 1.7, marginBottom: 16 }}>{p.desc}</p>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 700, color: '#ffffff', background: 'rgba(34,197,94,0.08)', padding: '6px 14px', borderRadius: 100 }}>
            ...
            <div className="reveal reveal-d1" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 480px), 1fr))', gap: 20 }}>
              {testimonials.map((t, i) => (
                <div key={t.name} className={`reveal reveal-d${Math.min(i + 1, 3)} loc-testimonial-card`} style={{ ...cardStyle, padding: '44px 36px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}
                  ...
                  {/* Quote icon */}
                  <div>
                    <svg width="32" height="32" viewBox="0 0 24 24" fill="rgba(34,197,94,0.3)" style={{ marginBottom: 20 }}><path d="M6 17h3l2-4V7H5v6h3l-2 4zm8 0h3l2-4V7h-6v6h3l-2 4z" /></svg>
                    <p style={{ fontSize: 16, color: 'rgba(255,255,255,0.7)', lineHeight: 1.8, fontStyle: 'italic', marginBottom: 28 }}>
                      &ldquo;{t.quote}&rdquo;
                    </p>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                    <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'rgba(17,24,39,0.12)', border: '1px solid rgba(34,197,94,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700, color: '#ffffff', flexShrink: 0 }}>
        ...
        <section ref={industryRef} style={{ ...sectionPad, ...sectionBorder }}>
          <div className="cb-container">
            ...
            <div className="reveal reveal-d1" style={{ marginBottom: 64 }}>
              <p style={{ fontSize: 17, color: 'rgba(255,255,255,0.7)', lineHeight: 1.85, maxWidth: 800 }}>
                {industryExpertise}
              </p>
            </div>
            ...
            <div className="reveal reveal-d2" style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 64 }}>
              {localIndustries.map(ind => (
                <span key={ind} style={{ padding: '10px 22px', border: '1px solid rgba(34,197,94,0.2)', borderRadius: 100, fontSize: 14, fontWeight: 600, color: '#ffffff', background: 'rgba(34,197,94,0.06)', transition: '0.25s' }}
                  onMouseEnter={e => { e.currentTarget.style.background = 'rgba(17,24,39,0.12)'; e.currentTarget.style.borderColor = 'rgba(34,197,94,0.4)'; }}
            ...
            <div className="reveal reveal-d3" style={{ position: 'relative', overflow: 'hidden', borderRadius: 32, border: '1px solid rgba(34,197,94,0.15)', background: 'linear-gradient(135deg, rgba(34,197,94,0.06) 0%, rgba(6,182,212,0.04) 100%)', padding: 'clamp(40px, 6vw, 64px)' }}>
              <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: 'linear-gradient(90deg, #22c55e, #4ade80)' }} />
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 320px), 1fr))', gap: 40, alignItems: 'center' }}>
                <div>
                  <h3 style={{ fontSize: 'clamp(1.6rem, 3vw, 2.4rem)', fontWeight: 600, color: '#ffffff', letterSpacing: '-0.03em', lineHeight: 1.15, marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  {[
                    ...
                    <div key={item.title} style={{ display: 'flex', gap: 16, padding: '20px 24px', borderRadius: 20, background: 'rgba(255,255,255,0.7)', border: '1px solid rgba(255,255,255,0.04)' }}>
                      <span style={{ fontSize: 24, flexShrink: 0 }}>{item.icon}</span>
                      <div>
                        <div style={{ fontSize: 14, fontWeight: 600, color: '#ffffff', marginBottom: 4 }}>{item.title}</div>
        ...
        <section ref={faqRef} style={{ ...sectionPad, ...sectionBorder }}>
          <div className="cb-container" style={{ maxWidth: 860 }}>
            ...
            <div className="reveal reveal-d1" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {faqs.map((faq, i) => (
                <div key={i}
                  style={{ background: openFaq === i ? 'rgba(34,197,94,0.04)' : 'rgba(255,255,255,0.015)', border: `1px solid ${openFaq === i ? 'rgba(34,197,94,0.2)' : 'rgba(255,255,255,0.05)'}`, borderRadius: 20, overflow: 'hidden', transition: 'border-color 0.3s, background 0.3s' }}>
                  ...
                  <div
                    ...
                    <p className="loc-faq-answer" style={{ padding: '0 28px 24px', margin: 0, fontSize: 15, color: 'rgba(255,255,255,0.4)', lineHeight: 1.75 }}>{faq.a}</p>
                  </div>
                </div>
              ))}
            </div>
