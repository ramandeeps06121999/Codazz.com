    ...
    <>
      ...
      <main style={{ background: '#000000', color: '#ffffff', paddingTop: 80 }}>
        ...
        <section ref={heroRef} style={{ position: 'relative', overflow: 'hidden', minHeight: '80vh', display: 'flex', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          <div className="cb-container" style={{ position: 'relative', zIndex: 1, textAlign: 'center', maxWidth: 900, margin: '0 auto', padding: 'clamp(60px, 10vw, 120px) 0' }}>
            ...
            <nav className="reveal" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 8, marginBottom: 28, flexWrap: 'wrap' }}>
              ...
              <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.7)' }}>{country.name}</span>
            </nav>
            ...
            <h1 className="reveal reveal-d1" style={{ fontSize: 'clamp(2.6rem, 5.5vw, 5rem)', fontWeight: 500, color: '#ffffff', letterSpacing: '-0.04em', lineHeight: 1.05, margin: '0 0 24px' }}>
              Software Development in{' '}<span style={{ color: '#ffffff' }}>{country.name}</span>
            </h1>
            ...
            {/* Stats row */}
            <div className="reveal reveal-d4 loc-hero-stats" style={{ display: 'grid', gridTemplateColumns: `repeat(${country.stats.length}, 1fr)`, gap: 16, maxWidth: 700, margin: '0 auto' }}>
              {country.stats.map((s, i) => (
                <div key={s.label} style={{ textAlign: 'center', borderRight: i < country.stats.length - 1 ? '1px solid rgba(255,255,255,0.06)' : 'none', paddingRight: i < country.stats.length - 1 ? 16 : 0 }}>
        ...
        {/* ════════════════════════════════════════════
            2. CITIES WE SERVE
        ════════════════════════════════════════════ */}
        <section ref={citiesRef} style={{ ...sectionPad, ...sectionBorder }}>
          <div className="cb-container">
            <div style={{ textAlign: 'center', marginBottom: 56 }}>
              <div className="reveal" style={subLabel}>Cities We Serve</div>
              <h2 className="reveal" style={heading2}>
                Our Locations in <span style={{ color: '#ffffff' }}>{country.name}</span>
              </h2>
              <p className="reveal" style={{ ...bodyText, maxWidth: 600, margin: '16px auto 0', color: 'rgba(255,255,255,0.7)' }}>
                Explore the cities where Codazz delivers world-class software development across {country.name}.
              </p>
            </div>
              ...
              {countryCities.map((city, i) => (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                      <div style={{ fontSize: 18, fontWeight: 600, color: '#ffffff', letterSpacing: '-0.01em' }}>{city.name}</div>
                      <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', marginTop: 4 }}>{city.state}</div>
                    </div>
                    <svg style={{ flexShrink: 0, opacity: 0.3 }} width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M12 5l7 7-7 7" /></svg>
                  </div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {city.localIndustries.slice(0, 4).map((industry) => (
        ...
        <section ref={servicesRef} style={{ ...sectionPad, ...sectionBorder }}>
          <div className="cb-container">
            <div style={{ textAlign: 'center', marginBottom: 56 }}>
              ...
              <h2 className="reveal" style={heading2}>
                What We Build in <span style={{ color: '#ffffff' }}>{country.name}</span>
              </h2>
              <p className="reveal" style={{ ...bodyText, maxWidth: 600, margin: '16px auto 0', color: 'rgba(255,255,255,0.7)' }}>
                Full-spectrum software development tailored to {country.name}&apos;s business landscape.
              </p>
            </div>
            <div className="loc-services-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 260px), 1fr))', gap: 16 }}>
              {services.map((svc, i) => (
                <Link
                  key={svc.slug}
                  href={`/locations/${primaryCity.slug}/${svc.slug}`}
                  className={`reveal reveal-d${(i % 4) + 1}`}
                  style={{
                    ...cardStyle,
                    padding: '28px 28px',
                ...
                >
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', marginTop: 4 }}>in {country.name}</div>
                  </div>
        ...
        <section ref={ctaRef} style={{ ...sectionPad, ...sectionBorder, position: 'relative', overflow: 'hidden' }}>
          ...
          <div className="cb-container" style={{ position: 'relative', zIndex: 1, textAlign: 'center', maxWidth: 760, margin: '0 auto' }}>
            ...
            <h2 className="reveal" style={{ ...heading2, marginBottom: 20 }}>
              Ready to Build Something Great in <span style={{ color: '#ffffff' }}>{country.name}</span>?
            </h2>
            <p className="reveal" style={{ ...bodyText, maxWidth: 560, margin: '0 auto 40px', color: 'rgba(255,255,255,0.7)' }}>
              Whether you need a mobile app, a web platform, or an AI-powered solution, our {country.name} team is ready to bring your vision to life.
            </p>
