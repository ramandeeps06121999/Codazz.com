    ...
    <>
      ...
      <main style={{ background: '#000000', color: '#ffffff', paddingTop: 80 }}>
        ...
        <section ref={heroRef} className="section-padding loc-city-hero" style={{ position: 'relative', overflow: 'hidden', minHeight: '70vh', display: 'flex', alignItems: 'center' }}>
          ...
          <div style={{ position: 'absolute', top: '20%', left: '50%', transform: 'translateX(-50%)', width: 600, height: 600, background: 'radial-gradient(circle, rgba(34,197,94,0.08) 0%, transparent 70%)', zIndex: 0, pointerEvents: 'none' }} />
        ...
        <section ref={s1} className="section-padding">
          <div className="cb-container">
            <div className="reveal" style={{ textAlign: 'center', marginBottom: '3rem' }}>
              ...
              <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '1.1rem' }}>Select a city to learn more about our local presence and services.</p>
            </div>
                ...
                <Link key={city.slug} href={`/locations/${city.slug}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                  <Card style={{ cursor: 'pointer', height: '100%' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                      ...
                      {city.isHQ && (
                        <span style={{ background: 'rgba(34,197,94,0.1)', borderRadius: 8, padding: '4px 12px', fontSize: 12, color: '#ffffff', letterSpacing: '0.05em', whiteSpace: 'nowrap' }}>HQ</span>
                      )}
                    </div>
                    <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.85rem', marginBottom: '0.5rem' }}>{({ UAE: 'UAE', UK: `${city.state}, UK`, AU: `${city.state}, Australia`, CA: `${city.state}, Canada`, SA: 'Saudi Arabia', QA: 'Qatar', SG: 'Singapore', DE: `${city.state}, Germany`, IN: `${city.state}, India`, JP: 'Japan', KR: 'South Korea', NL: 'Netherlands', IE: 'Ireland', IL: 'Israel', PL: 'Poland', BR: 'Brazil', MX: 'Mexico', NG: 'Nigeria', KE: 'Kenya', VN: 'Vietnam', EG: 'Egypt', NZ: 'New Zealand', CH: 'Switzerland' } as Record<string, string>)[city.country] || city.state}</p>
                    <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.95rem', lineHeight: 1.6 }}>{city.tagline}</p>
                    <div style={{ color: '#ffffff', fontSize: '0.9rem', marginTop: '1rem', fontWeight: 600 }}>Learn more &rarr;</div>
                  </Card>
                </Link>
              ))}
            </div>
          ...
          <div className="cb-container">
            <div className="reveal" style={{ textAlign: 'center', maxWidth: 700, margin: '0 auto' }}>
              <h2 style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 800, lineHeight: 1.15, marginBottom: '1.5rem' }}>
                Ready to Build <span style={{ color: '#ffffff' }}>Something Great?</span>
              </h2>
              ...
              <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
                ...
                <Link href="/services/mobile-app-development" style={{ border: '1px solid rgba(255,255,255,0.1)', color: '#ffffff', padding: '16px 36px', borderRadius: 999, fontWeight: 600, fontSize: '1rem', textDecoration: 'none', display: 'inline-block' }}>
                  Explore Services
                </Link>
              </div>
            </div>
