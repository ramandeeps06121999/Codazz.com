...
export default function ContactPage() {
  ...
  const inp: React.CSSProperties = { width: '100%', padding: '15px 20px', fontSize: 16, fontFamily: 'inherit', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, color: '#ffffff', outline: 'none', transition: 'border 0.25s, box-shadow 0.25s', boxSizing: 'border-box' };
  const sel: React.CSSProperties = { ...inp, appearance: 'none' as const, backgroundImage: `url("data:image/svg+xml,%3Csvg width='12' height='7' viewBox='0 0 12 7' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 1L6 6L11 1' stroke='rgba(255,255,255,0.5)' stroke-width='1.5' stroke-linecap='round'/%3E%3C/svg%3E")`, backgroundRepeat: 'no-repeat', backgroundPosition: 'right 18px center' };
  const lbl: React.CSSProperties = { display: 'block', fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase' as const, color: 'rgba(255,255,255,0.4)', marginBottom: 8 };
  const fi = (e: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => { e.currentTarget.style.borderColor = 'rgba(34,197,94,0.5)'; e.currentTarget.style.boxShadow = '0 0 0 3px rgba(34,197,94,0.08)'; };
  const fo = (e: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.08)'; e.currentTarget.style.boxShadow = 'none'; };

  return (
    <div style={{ background: '#000000', color: '#ffffff', paddingTop: 80 }}>

      {/* ═══════════════════════════════════════
          HERO
      ═══════════════════════════════════════ */}
      <section ref={heroRef} style={{ position: 'relative', overflow: 'hidden', padding: 'clamp(60px, 10vw, 100px) 0 clamp(60px, 10vw, 120px)', borderBottom: '1px solid rgba(255,255,255,0.06)', textAlign: 'center' }}>
        ...
        <div className="cb-container" style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 28 }}>
          ...
          <h1 className="reveal reveal-d1" style={{ fontSize: 'clamp(3rem,8vw,8rem)', fontWeight: 500, color: '#ffffff', letterSpacing: '-0.05em', lineHeight: 0.95, margin: 0, maxWidth: 1000 }}>
          ...
          {/* Trust strip */}
          <div className="reveal reveal-d4" style={{ display: 'flex', gap: 32, flexWrap: 'wrap', paddingTop: 44, borderTop: '1px solid rgba(255,255,255,0.06)', justifyContent: 'center', width: '100%', marginTop: 8 }}>
            {TRUST.map(t => (
      ...
      {/* ═══════════════════════════════════════
          ENGAGEMENT OPTIONS
      ═══════════════════════════════════════ */}
      <section ref={engageRef} style={{ padding: 'clamp(48px, 8vw, 100px) 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div className="cb-container">
          <div className="reveal" style={{ textAlign: 'center', marginBottom: 60 }}>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase' as const, color: 'rgba(255,255,255,0.25)', marginBottom: 16 }}>How Can We Help?</div>
            <h2 style={{ fontSize: 'clamp(2.2rem,4vw,3.8rem)', fontWeight: 500, color: '#ffffff', letterSpacing: '-0.04em', lineHeight: 1.08, margin: 0 }}>
          ...
          <div className="reveal reveal-d1 cp-engage-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(280px, 100%), 1fr))', gap: 20 }}>
            {ENGAGEMENT.map(o => (
              <a key={o.num} href={'href' in o && o.href ? o.href : '#contact-form'} {...('href' in o && o.href ? { target: '_blank', rel: 'noopener noreferrer' } : {})}
                style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 36, padding: 'clamp(28px, 4vw, 52px) clamp(24px, 3.5vw, 44px)', display: 'flex', flexDirection: 'column', gap: 20, position: 'relative', overflow: 'hidden', textDecoration: 'none', transition: 'all 0.4s cubic-bezier(0.165,0.84,0.44,1)' }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(34,197,94,0.28)'; e.currentTarget.style.background = 'rgba(34,197,94,0.04)'; e.currentTarget.style.transform = 'translateY(-6px)'; e.currentTarget.style.boxShadow = '0 32px 64px rgba(0,0,0,0.3)'; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.06)'; e.currentTarget.style.background = 'rgba(255,255,255,0.03)'; e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = ''; }}>
                {/* Ghost watermark */}
                <div style={{ position: 'absolute', bottom: -20, right: 24, fontSize: 130, fontWeight: 800, lineHeight: 1, color: 'rgba(255,255,255,0.02)', pointerEvents: 'none', letterSpacing: '-0.05em' }}>{o.num}</div>
                <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase' as const, color: '#22c55e', background: 'rgba(34,197,94,0.1)', padding: '5px 14px', borderRadius: 100, display: 'inline-block', alignSelf: 'flex-start' }}>{o.tag}</span>
                <div>
                  <h3 style={{ fontSize: 22, fontWeight: 600, color: '#ffffff', letterSpacing: '-0.02em', marginBottom: 12 }}>{o.title}</h3>
                  <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.4)', lineHeight: 1.7, margin: 0 }}>{o.desc}</p>
                </div>
                <div style={{ marginTop: 'auto', display: 'inline-flex', alignItems: 'center', gap: 8, color: '#22c55e', fontSize: 13, fontWeight: 700 }}>{o.cta}</div>
              </a>
            ))}
          </div>
        </div>
      ...
      {/* ═══════════════════════════════════════
          CONTACT FORM
      ═══════════════════════════════════════ */}
      <section id="contact-form" ref={formRef} style={{ padding: 'clamp(48px, 8vw, 100px) 0', borderBottom: '1px solid rgba(255,255,255,0.06)', scrollMarginTop: 80 }}>
        <div className="cb-container">
          <div className="reveal cp-contact-form-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1.4fr', gap: 'clamp(32px, 6vw, 80px)', alignItems: 'start' }}>
            ...
            {/* Left info col */}
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase' as const, color: 'rgba(255,255,255,0.25)', marginBottom: 20 }}>Get in Touch</div>
              <h2 style={{ fontSize: 'clamp(2rem,3.5vw,3.2rem)', fontWeight: 500, color: '#ffffff', letterSpacing: '-0.04em', lineHeight: 1.1, marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 24, marginBottom: 48 }}>
                {[
                  ...
                  <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    <div style={{ width: 44, height: 44, borderRadius: 12, background: 'rgba(34,197,94,0.07)', border: '1px solid rgba(34,197,94,0.13)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#22c55e' }} />
                    </div>
                    <div>
                      <div style={{ fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.25)', textTransform: 'uppercase' as const, letterSpacing: '0.1em', marginBottom: 3 }}>{item.label}</div>
                      {item.href
                        ? <a href={item.href} style={{ fontSize: 14, color: '#ffffff', fontWeight: 500, textDecoration: 'none' }}>{item.value}</a>
              ...
              {/* NDA note */}
              <div style={{ padding: '20px 22px', background: 'rgba(34,197,94,0.04)', border: '1px solid rgba(34,197,94,0.1)', borderRadius: 16, display: 'flex', alignItems: 'flex-start', gap: 14 }}>
            ...
            {/* Right form */}
            <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 36, padding: 'clamp(20px, 4vw, 52px) clamp(20px, 4vw, 44px)' }}>
              {submitted ? (
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                  <div style={{ marginBottom: 4 }}>
                    ...
                    <h3 style={{ fontSize: 24, fontWeight: 600, color: '#ffffff', letterSpacing: '-0.03em', margin: 0 }}>Start the Conversation</h3>
                  </div>
                  ...
                  <div>
                    ...
                    <select id="cp-projectType" name="projectType" value={form.projectType} onChange={handle} required aria-required="true" style={{ ...sel, color: form.projectType ? '#ffffff' : 'rgba(255,255,255,0.25)' }} onFocus={fi} onBlur={fo}>
                  ...
                  <div>
                    ...
                    <select id="cp-timeline" name="timeline" value={form.timeline} onChange={handle} style={{ ...sel, color: form.timeline ? '#ffffff' : 'rgba(255,255,255,0.25)' }} onFocus={fi} onBlur={fo}>
                      <option value="" disabled>When do you need this?</option>
                      {TIMELINES.map(t => <option key={t} value={t} style={{ background: '#0a0a0a', color: '#ffffff' }}>{t}</option>)}
                    </select>
                  </div>
                  ...
                  {error && <div id="cp-form-error" role="alert" style={{ color: '#22c55e', fontSize: 13, fontWeight: 500, background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.2)', padding: '12px 16px', borderRadius: 12, marginTop: 4 }}>{error}</div>}

                  <button type="submit" disabled={isLoading} style={{ marginTop: 4, height: 60, borderRadius: 100, border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, background: isLoading ? 'rgba(34,197,94,0.5)' : '#22c55e', color: '#000', fontSize: 15, fontWeight: 700, cursor: isLoading ? 'not-allowed' : 'pointer', transition: '0.3s', fontFamily: 'inherit' }}
      ...
      ═══════════════════════════════════════ */}
      <section ref={officesRef} style={{ padding: 'clamp(48px, 8vw, 100px) 0', borderBottom: '1px solid rgba(255,255,255,0.06)', position: 'relative', overflow: 'hidden' }}>
        {/* Subtle glow */}
        <div style={{ position: 'absolute', top: '40%', left: '50%', transform: 'translate(-50%,-50%)', width: 800, height: 500, background: 'radial-gradient(ellipse,rgba(34,197,94,0.06) 0%,transparent 65%)', filter: 'blur(80px)', pointerEvents: 'none' }} />

        <div className="cb-container" style={{ position: 'relative', zIndex: 1 }}>
          <div className="reveal" style={{ textAlign: 'center', marginBottom: 56 }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: 100, padding: '8px 20px', marginBottom: 24 }}>
              <span style={{ fontSize: 14 }}>🖥️</span>
              <span style={{ fontSize: 11, fontWeight: 700, color: '#22c55e', letterSpacing: '0.12em', textTransform: 'uppercase' as const }}>99% Virtual Meetings</span>
            </div>
            <h2 style={{ fontSize: 'clamp(2.2rem,4vw,3.8rem)', fontWeight: 500, color: '#ffffff', letterSpacing: '-0.04em', lineHeight: 1.08, margin: '0 0 20px' }}>
          ...
          <div className="reveal reveal-d1" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(240px, 100%), 1fr))', gap: 16, marginBottom: 56 }}>
            {VIRTUAL_PERKS.map(p => (
              <div key={p.title} style={{ padding: 'clamp(24px, 3vw, 36px)', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 24, transition: '0.4s' }}
                ...
                <h3 style={{ fontSize: 17, fontWeight: 600, color: '#ffffff', letterSpacing: '-0.02em', marginBottom: 8 }}>{p.title}</h3>
                <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.4)', lineHeight: 1.7, margin: 0 }}>{p.desc}</p>
              </div>
            ))}
          </div>
      ...
      {/* ═══════════════════════════════════════
          FAQ
      ═══════════════════════════════════════ */}
      <section ref={faqRef} style={{ padding: 'clamp(48px, 8vw, 100px) 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div className="cb-container" style={{ maxWidth: 860 }}>
          ...
          <div className="reveal reveal-d1" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {FAQS.map((faq, i) => (
              <div key={i} style={{ background: openFaq === i ? 'rgba(34,197,94,0.04)' : 'rgba(255,255,255,0.03)', border: `1px solid ${openFaq === i ? 'rgba(34,197,94,0.2)' : 'rgba(255,255,255,0.06)'}`, borderRadius: 20, overflow: 'hidden', transition: 'border-color 0.3s, background 0.3s' }}>
                ...
                {openFaq === i && (
                  <p style={{ padding: '0 clamp(16px, 3vw, 28px) clamp(16px, 3vw, 24px)', margin: 0, fontSize: 15, color: 'rgba(255,255,255,0.4)', lineHeight: 1.75 }}>{faq.a}</p>
                )}
              </div>
            ))}
          </div>
