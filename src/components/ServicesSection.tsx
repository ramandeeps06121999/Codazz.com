        ...
        <div className="services-large-grid" style={{ marginBottom: 20 }}>
          {large.map((s, i) => (
            <div key={s.title} className={`reveal reveal-d${i + 1}`}
              ...
              onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(79,70,229,0.25)'; e.currentTarget.style.background = 'rgba(79,70,229,0.03)'; e.currentTarget.style.boxShadow = '0 20px 60px rgba(0,0,0,0.5), 0 0 0 1px rgba(79,70,229,0.1)'; e.currentTarget.style.transform = 'translateY(-4px)'; }}
            ...
            >
              ...
              <div>
                <span style={{ display: 'inline-block', fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#4F46E5', background: 'rgba(79,70,229,0.1)', padding: '6px 14px', borderRadius: 100, marginBottom: 24 }}>{s.tag}</span>
                ...
                <p style={{ fontSize: 16, color: 'rgba(0,0,0,0.55)', lineHeight: 1.7, margin: 0, maxWidth: 440 }}>{s.desc}</p>
              </div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginTop: 'auto' }}>
                {s.chips.map(c => <span key={c} style={{ fontSize: 12, fontWeight: 600, color: 'rgba(0,0,0,0.55)', padding: '7px 16px', border: '1px solid rgba(0,0,0,0.06)', borderRadius: 100 }}>{c}</span>)}
              </div>
              {s.href
                ? <Link href={s.href} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, color: '#4F46E5', fontSize: 13, fontWeight: 700, letterSpacing: '0.06em', textDecoration: 'none' }}>EXPLORE SERVICE <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M5 12h14M12 5l7 7-7 7" /></svg></Link>
        ...
        <div className="services-small-grid">
          {small.map((s, i) => (
            ...
            >
              ...
              <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#4F46E5' }}>{s.tag}</span>
              <div>
